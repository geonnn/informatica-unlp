...
public class BancoITAU {
	...
	public List<Jubilado> carteraJubilado (Graph<Persona> grafo, String nomEmpleado, int grado, int maxList){
		List<Jubilado> jubilados = new LinkedList<Jubilado>();
		if (grafo != null){// No puede ir en el mismo if que isEmpty() porque rompe todo. 
			if (!grafo.isEmpty()) {
				...
				if(emp != null) {
					...
					while (!c.isEmpty() && grado > 0 && jubilados.size() < maxList) {
						...
						if (v != null) {
							...
							while (it.hasNext() && grado > 0 && jubilados.size() < maxList) {
